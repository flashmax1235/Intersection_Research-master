...
class Reservation:
    ...
    def toString(self):
        return ("[ Vin: " + str(self.vin) + " ,speed: " + str(self.speed) + " ,accel: " +  str(self.accel) + " ,Entered time: " + str(self.enterTime) + " ,Lane #: " + str(self.lane)  + " ,Expected time #: " + str(self.expectedTime)  + " ,requested accel #: " + str(self.requestedAccel) +"]")
...
class Intersection:  
        ...
        if (self.check_avalability_initial(new_node)):
            ...
            if(cursor == None): # nothing to worry about on the right
                ...
                if(goal[0] < goal[1]) and can_left[0]:  # less energy to get to left and witin criteria
                    ...
                    res.expectedTime = left.expectedTime - inter.inter_tolerance_time
                    ...
                    res.expectedTime = right.expectedTime + inter.inter_tolerance_time
    ...
    def withinCriteriaLeft(self,res,option): #option1 is left search, option2 is right search
        opt1_time = (option.expectedTime - inter.inter_tolerance_time) - option.enterTime
        left = (2*(inter.inter_side_length - res.speed *opt1_time))/(opt1_time ** 2)

        if(left < 0) and (left > inter.car_max_decel): #check if pos or neg, compare to mac accel/dec and update res
            #print "left deceleration approved!"
            return  True, left
        elif (left > 0) and (left < inter.car_max_accel):
            print
    ...
    def withinCriteriaRight(self, res, option):
        optionTime = (option.expectedTime + inter.inter_tolerance_time) - option.enterTime
        right = (2 * (inter.inter_side_length - res.speed * optionTime)) / (optionTime ** 2)
        if (right < 0) and (right > inter.car_max_decel):  # check if pos or neg, compare to mac accel/dec and update res
            print "right deceleration approved!"
            return True, right
        elif (right > 0) and (right < inter.car_max_accel):
            print
